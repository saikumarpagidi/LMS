...
public class PageController {
	...
	public String processRegister(@Valid @ModelAttribute("userForm") UserForms userForm, BindingResult bindingResult,
		...
		User user = User.builder().name(userForm.getName()).email(userForm.getEmail()).password(userForm.getPassword())
				.phoneNumber(userForm.getPhoneNumber()).college(userForm.getCollege()) // Store college name directly
...
//        // Save the user AND assign STUDENT role in one transaction
//        userService.saveUserAndAssignRole(user, "STUDENT");

		// Add a success message to the session
		Message message = Message.builder().content("Registration Successful").type(MessageType.green) // GREEN for
																										// success
				.build();
