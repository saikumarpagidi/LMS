...
public class FacultyDashboardController {
	...
	public FacultyDashboardController(CourseAssignmentService courseAssignmentService,
	...
	public String facultyDashboard(Model model, Authentication authentication) {
		User faculty = (User) authentication.getPrincipal();
		if (!faculty.hasRole("FACULTY")) {
			return "redirect:/user/dashboard";
		}

		// Fetch course assignments for the faculty
		List<CourseAssignment> assignments = courseAssignmentService.getAssignmentsForFaculty(faculty);
		...
		assignments.forEach(assignment -> {
			...
			assignment.getCourse().setModules(modules != null ? modules : new ArrayList<>());
	...
	@GetMapping("/faculty/course-syllabus/{courseId}")
	public String showFacultyCourseSyllabus(@PathVariable Integer courseId, Model model,
